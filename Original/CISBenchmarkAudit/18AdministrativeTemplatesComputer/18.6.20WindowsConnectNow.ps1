...
.DESCRIPTION
This policy setting allows the configuration of wireless settings using Windows Connect Now (WCN). The WCN Registrar enables the discovery and configuration of devices over Ethernet (UPnP) over in-band 802.11 Wi-Fi through the Windows Portable Device API (WPD) and via USB Flash drives.
...
General notes
#>
function Test-WindowsConnectNowEnableRegistrars {
    [CmdletBinding()]
    param (
...
General notes
#>
function Test-WindowsConnectNowDisableWcnUi {
    [CmdletBinding()]
    param (
        # Get the product type (1, 2 or 3)
        [Parameter()][ValidateSet(1,2,3)][int]$ProductType = (Get-ProductType),
