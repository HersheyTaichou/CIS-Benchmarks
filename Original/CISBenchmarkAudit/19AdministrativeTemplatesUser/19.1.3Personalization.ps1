...
General notes
#>
function Test-UserPersonalizationScreenSaveActive {
    [CmdletBinding()]
    param (
...
General notes
#>
function Test-UserPersonalizationScreenSaverIsSecure {
    [CmdletBinding()]
    param (
        # Get the product type (1, 2 or 3)
        [Parameter()][ValidateSet(1,2,3)][int]$ProductType = (Get-ProductType),
...
General notes
#>
function Test-UserPersonalizationScreenSaveTimeOut {
    [CmdletBinding()]
    param (
        # Get the product type (1, 2 or 3)
        [Parameter()][ValidateSet(1,2,3)][int]$ProductType = (Get-ProductType),
    ...
    process {
        ...
        if (($Result.Setting -le 900 -and $Result.Setting -gt 0) -and $Result.Entry.State -eq "Enabled") {
            $Result.SetCorrectly = $true
        } else {
            $Result.SetCorrectly = $false
        }
